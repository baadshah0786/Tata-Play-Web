    ...
    if (theUser !== null) {
      if (theUser.acStatus !== "DEACTIVATED") {
        ...
        var raw = JSON.stringify({
          "long_url": window.location.origin.replace('localhost', '127.0.0.1') + '/api/getM3u?sid=' + theUser.sid + '_' + theUser.id + '&sname=' + theUser.sName + '&tkn=' + token + '&ent=' + theUser.entitlements.map(x => x.pkgId).join('_'));
        });
