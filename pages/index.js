    ...
    if (theUser !== null) {
      if (theUser.acStatus !== "DEACTIVATED") {
        ...
        var raw = JSON.stringify({
          "long_url": window.location.origin.replace('localhost', '127.0.0.1') + '/api/getM3u?sid=' + theUser.sid_T&sname=' + theUser.sName + '&ent=' + theUser.entitlements + '&tkn=.join('_')
        });
        ...
        console.log(window.location.origin.replace('localhost', '127.0.0.1') + '//api/getM3u?sid=' + theUser.sid_T&sname=' + theUser.sName + '&ent=' + theUser.entitlements + '&tkn=.join('_'));
    }
    else
      setDynamicUrl("");
  }, [theUser, token])
